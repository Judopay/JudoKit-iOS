...
@preconcurrency import JudoKit_iOS

enum JPResult: Sendable {
    case success(JPResponse)
    case failure(JPError)
}
...
class FeaturesViewController: UIViewController {
    ...
    @MainActor
    private func invokeBasicApplePayAsyncAwait(_ isPreauth: Bool) async -> (JPResult) {
        await withCheckedContinuation { [unowned self] continuation in
            self.judoKit?.invokeApplePay(with: isPreauth ? .preAuth : .payment,
                ...
                let result: JPResult = error == nil ? .success(response!) : .failure(error!)
    ...
    @MainActor
    private func presentApplePayViewControllerAsyncAwait(_ isPreauth: Bool) async -> (JPResult) {
        await withCheckedContinuation { [unowned self] continuation in
            self.judoKit?.invokeApplePay(with: isPreauth ? .preAuth : .payment,
                                         configuration: configuration,
                                         presenting: self,
    ...
    private func invokeBasicApplePayAsyncAwait(_ isPreauth: Bool) {
        Task { [unowned self] in
            let response = await invokeBasicApplePayAsyncAwait(isPreauth)
            switch response {
            case .failure(let error):
                self.onComplete(withResponse: nil, and: error)
            case .success(let response):
                self.onComplete(withResponse: response, and: nil)
            }
        }
    }
    ...
    private func invokeCustomApplePayAsyncAwait(_ isPreauth: Bool) {
        Task { [unowned self] in
            let response = await presentApplePayViewControllerAsyncAwait(isPreauth)
            switch response {
            case .failure(let error):
                self.onComplete(withResponse: nil, and: error)
            case .success(let response):
    ...
    private func invokeCustomApplePay(_ isPreauth: Bool) {
        self.judoKit?.invokeApplePay(with: isPreauth ? .preAuth : .payment,
                                     configuration: configuration, presenting: self, completion: { [unowned self] response, error in
            self.onComplete(withResponse: response, and: error)
        })
    }
